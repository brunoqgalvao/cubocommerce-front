...
const FormItem = (props) => {  
  ...
  const handleAdd = () => {
    ...
    showForSeconds(`${product.name} adicionado!`,1000, {vertical:'bottom',horizontal:'center',variant:'primary'});
  }
  const handleRemove = () => {
    removeOrder(product.id);
    showForSeconds(`Item Removido!`,1000, {vertical:'bottom',horizontal:'center',variant:'primary'});
  ...
  return (
    <>
          ...
          >
          <FormCard product={product} handleAdd={handleAdd} handleRemove={handleRemove}/>
        </Modal>
        </>
  )
}
